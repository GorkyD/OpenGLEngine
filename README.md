...
### 5) Math conventions (`Matrix4`/`Quaternion`)

`Math/Matrix4.h` uses a row-vector convention: `v' = v * M`, matrices are row-major (`matrix[row][col]`), and translation lives in the last row (`matrix[3][0..2]`). Composing transforms therefore reads left-to-right in application order, e.g. `TransformComponent::GetModelMatrix()` returns `scale * rotation * translation` (scale applied first, translation last). `Quaternion::ToMatrix4()` produces a matrix in this same convention — it is the transpose of the textbook column-vector quaternion-to-matrix formula, verified against `Matrix4::SetRotationX/Y/Z`.

This matters most in `Resource/SkinnedModelLoader.cpp`: Assimp's `aiMatrix4x4` is column-vector (`v' = M * v`, translation in the last *column*), so `ConvertMatrix()` transposes on the way in. Per-bone final matrices are then composed as `boneOffset * nodeGlobal * globalInverseTransform` (row-vector order), consumed in `SimpleShader_Skinned.vert` the same way `world` is consumed in `SimpleShader_Lit.vert` (`M * vec4(position, 1.0)` in GLSL, uploaded with the implicit row-major→column-major transpose).
